...
* **INSTRUCTIONS**:
  ...
  1) Open the folder slacked out to you, run `npm install` Check

  2) Create a new MySQL database and name it `blogger`. Don't create any tables. Check

  3) Open the `config` folder and update the `config.json` file's `development` object to match your own local MySQL database. Check
