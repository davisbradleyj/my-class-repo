...
d3.csv("heights_of_rollercoasters.csv", function(error, coasterData) {
  ...
  var yLinearScale = d3.scaleLinear()
    .domain([0, d3.max(coasterData, d => d.height)])
    .range([chartHeight, 0]);
  ...
  // Create one SVG rectangle per piece of coasterData
  ...
  chartGroup.selectAll(".bar")
    ...
    .append("rect")
    .attr("class", "bar")
    .attr("x", d => xBandScale(d.rollercoaster))
    .attr("y", d => yLinearScale(d.height))
    .attr("width", xBandScale.bandwidth())
    .attr("height", d => chartHeight - yLinearScale(d.height));
